...
?>
	<div class="container">	
		<form action="" method="POST">
			<h2 style="display: flex; float: left;">KONTRAK</h2> 
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">CETAK PDF DATA KONTRAK</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          </div>
          <div class="modal-body">
            <form target="_blank" action="CetakKontrakPDF.php" method="post">
              <div class="form-group">
                <label for="">Tanggal Awal</label>
                <input type="date" value="<?php $tgl = date('Y-m-d'); echo date('Y-m-d',strtotime($tgl.' -1 year'));  ?>" name="tgl_awal" class="form-control" required />
              </div>
              <div class="form-group">
		...
		<div class="table-responsive table-responsive-md table-responsive-sm table-responsive-lg" >
			<form name="kontrak_proses" method="POST">
			<div class="form-group">
			...
			<button type="button" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="Tambah"><a id="log" href="kontrak_input"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M19,13H13V19H11V13H5V11H11V5H13V11H19V13Z" /></svg></a></button>

			<div style="display: inline; float: right;">
				<button type="button" onclick="edit_kontrak()" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-warning"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M5,3C3.89,3 3,3.89 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V12H19V19H5V5H12V3H5M17.78,4C17.61,4 17.43,4.07 17.3,4.2L16.08,5.41L18.58,7.91L19.8,6.7C20.06,6.44 20.06,6 19.8,5.75L18.25,4.2C18.12,4.07 17.95,4 17.78,4M15.37,6.12L8,13.5V16H10.5L17.87,8.62L15.37,6.12Z" /></svg></a></button>
				<button type="button" onclick="hapus_kontrak()" data-toggle="tooltip" data-placement="top" title="Hapus" class="btn btn-danger"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M20.37,8.91L19.37,10.64L7.24,3.64L8.24,1.91L11.28,3.66L12.64,3.29L16.97,5.79L17.34,7.16L20.37,8.91M6,19V7H11.07L18,11V19A2,2 0 0,1 16,21H8A2,2 0 0,1 6,19Z" /></svg></a></button>
			</div>
		</div>
			<table class="table table-hover table-bordered table-sm">
				...
				<tr class="text-center">
					<th>No</th>
					<th>No. Kontrak <a id="log" href="kontrak_no_kontrak_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Nama Mitra <a id="log" href="kontrak_nama_cv_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Bagi Hasil <a id="log" href="kontrak_bagihasil_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Awal Tanggal <a id="log" href="kontrak_tanggal_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Batas Kontrak <a id="log" href="kontrak_tanggal1_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Status</th>
					<th><input type="checkbox" id="kontrak_select"></th>
				</tr>
				<?php
					$halaman 	  = 7; //batasan halaman
					$page    	  = isset($_GET['halaman'])? (int)$_GET["halaman"]:1;
					$mulai        = ($page>1) ? ($page * $halaman) - $halaman : 0;
					$sql     	  = mysqli_query($koneksi, 'SELECT * FROM kontrak');
					...
					if(isset($_POST['cari'])){
						$cari = $_POST['cari'];
						$kontrak = mysqli_query($koneksi, "SELECT * FROM kontrak INNER JOIN mitra ON kontrak.id_mitra = mitra.id_mitra WHERE nama_cv LIKE '%".$cari."%' OR tanggal LIKE '%".$cari."%' OR jatuh_tempo LIKE '%".$cari."%' OR surveyor LIKE '%".$cari."%' OR tonase LIKE '%".$cari."%' OR harga LIKE '%".$cari."%' ");
					}else{
						if(isset($_GET['urut'])){
							$kontrak = mysqli_query($koneksi, "SELECT * FROM kontrak INNER JOIN mitra ON kontrak.id_mitra = mitra.id_mitra ORDER BY $orderby $polabaru LIMIT $mulai, $halaman");
						}else{
							$kontrak = mysqli_query($koneksi, "SELECT * FROM kontrak INNER JOIN mitra ON kontrak.id_mitra = mitra.id_mitra LIMIT $mulai, $halaman");	
						}
					}

					$no = 1;

					if(mysqli_num_rows($kontrak)> 0){
						while($data = mysqli_fetch_array($kontrak)){ ?>
							<tr class="text-center">
								<td><?= $no++; ?></td>
								<td><?= $data['no_kontrak'] ?></td>
								<td><?= $data['nama_cv'] ?></td>
								<td><?= $data['bagihasil'] ?></td>
								<td><?php 
									$tgl = $data['tanggal'];
									echo tgl_indo($tgl);
								 ?></td>
								...
								<td>
									<input type="checkbox" name="checked[]" class="kontrak_check" value="<?= $data['id_kontrak'] ?>">
								</td>
							</tr>
						<?php }
					}else{

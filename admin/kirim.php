...
?>
	<div class="container">	
		<form action="" method="POST">
			<h2 style="display: flex; float: left;">PENGIRIMAN BATUBARA HARIAN</h2> 
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">CETAK PDF DATA PENGIRIMAN</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
          </div>
          <div class="modal-body">
            <form action="CetakKirimPDF.php" target="_blank" method="post">
              ...
              <div class="form-group">
                <label for="">Truk</label>
                <select name="kode_truk" class="form-control">
					...
					<?php
						...
						while($baris = mysqli_fetch_array($ahay)) { ?>
							<option value="<?= $baris[kode_truk] ?>"><?= $baris['kode_truk'] ?></option> 
						<?php } ?>
				</select>
              </div>
              <div class="form-group">
                ...
                <select name="no_transaksi" class="form-control">
                	<option value="">SEMUA</option>
					<?php
						$ahay = mysqli_query($koneksi, "SELECT * FROM transaksi INNER JOIN mitra ON transaksi.id_mitra = mitra.id_mitra");
						while($baris = mysqli_fetch_array($ahay)) { ?>
							<option value="<?= $baris[no_transaksi] ?>"><?= $baris['no_transaksi'].' - '.$baris['nama_cv'] ?></option> 
						<?php } ?>
				</select>
              </div>
              <div class="text-left">
		...
		<div class="table-responsive table-responsive-md table-responsive-sm table-responsive-lg">
			<form name="kirim_proses" method="POST">
				<div class="form-group">
			...
			<button type="button" data-toggle="tooltip" data-placement="top" title="Tambah" class="btn btn-success"><a id="log" href="kirim_input"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M19,13H13V19H11V13H5V11H11V5H13V11H19V13Z" /></svg></a></button>
			<div style="display: inline; float: right;">
				<button type="button" onclick="kirim_edit()" data-toggle="tooltip" data-placement="top" title="Ubah" class="btn btn-warning"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M5,3C3.89,3 3,3.89 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V12H19V19H5V5H12V3H5M17.78,4C17.61,4 17.43,4.07 17.3,4.2L16.08,5.41L18.58,7.91L19.8,6.7C20.06,6.44 20.06,6 19.8,5.75L18.25,4.2C18.12,4.07 17.95,4 17.78,4M15.37,6.12L8,13.5V16H10.5L17.87,8.62L15.37,6.12Z" /></svg></a></button>
			<button type="button" onclick="kirim_hapus()" data-toggle="tooltip" data-placement="top" title="Hapus" class="btn btn-danger"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="M20.37,8.91L19.37,10.64L7.24,3.64L8.24,1.91L11.28,3.66L12.64,3.29L16.97,5.79L17.34,7.16L20.37,8.91M6,19V7H11.07L18,11V19A2,2 0 0,1 16,21H8A2,2 0 0,1 6,19Z" /></svg></a></button>
			</div>
		</div>
			<table class="table table-hover table-bordered table-sm">
				...
				<tr class="text-center">
					<th>No</th>
					<th>Tanggal <a id="log" href="kirim_tgl_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Truk <a id="log" href="kirim_truk_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>No Surat Kirim <a id="log" href="kirim_nosurat_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Lahan<a id="log" href="kirim_lahan_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>Batu Bara (T) <a id="log" href="kirim_batubara_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th>No. Transaksi</th>
					<th>Nama Mitra</th>
					<th>Lokasi <a id="log" href="kirim_lokasi_<?=$polabaru;?>"><svg style="width:24px;height:24px" viewBox="0 0 24 24"><path fill="#FFFFFF" d="<?= $kodesvg ?>" /></svg></a></th>
					<th><input type="checkbox" id="select_kirim"></th>
				</tr>
				<?php
					$halaman 	  = 7; //batasan halaman
					$page    	  = isset($_GET['halaman'])? (int)$_GET["halaman"]:1;
					$mulai        = ($page>1) ? ($page * $halaman) - $halaman : 0;
					$sql     	  = mysqli_query($koneksi, "SELECT * FROM kirim 
								...
								INNER JOIN mitra ON mitra.id_mitra = transaksi.id_mitra");
					...
					if(isset($_POST['cari'])){
						$cari = $_POST['cari'];
						$kirim = mysqli_query($koneksi, "SELECT * FROM kirim 
								INNER JOIN stokpile ON kirim.id_stokpile = stokpile.id_stokpile 
								INNER JOIN truk ON kirim.id_truk = truk.id_truk
								INNER JOIN transaksi ON kirim.id_transaksi = transaksi.id_transaksi
								INNER JOIN mitra ON mitra.id_mitra = transaksi.id_mitra WHERE kode_truk LIKE '%$cari%' OR asal_batu LIKE '%$cari%' OR kode_lahan LIKE '%$cari%' OR muatan LIKE '%$cari%' OR kosong LIKE '%$cari%' ");
					}else{
						if(isset($_GET['urut'])){
							$kirim = mysqli_query($koneksi, "SELECT * FROM kirim 
								INNER JOIN stokpile ON kirim.id_stokpile = stokpile.id_stokpile 
								INNER JOIN truk ON kirim.id_truk = truk.id_truk
								INNER JOIN transaksi ON kirim.id_transaksi = transaksi.id_transaksi
								INNER JOIN mitra ON mitra.id_mitra = transaksi.id_mitra ORDER BY $orderby $polabaru LIMIT $mulai, $halaman");
						}else{
							$kirim = mysqli_query($koneksi, "SELECT * FROM kirim 
								INNER JOIN stokpile ON kirim.id_stokpile = stokpile.id_stokpile 
								INNER JOIN truk ON kirim.id_truk = truk.id_truk
								INNER JOIN transaksi ON kirim.id_transaksi = transaksi.id_transaksi
								INNER JOIN mitra ON mitra.id_mitra = transaksi.id_mitra  ORDER BY tanggal_kirim ASC LIMIT $mulai, $halaman");	
						}
					}
					...
					if(mysqli_num_rows($kirim)> 0){
						while($data = mysqli_fetch_array($kirim)){ ?>
							<tr class="text-center">
								<td><?= $no++; ?></td>
								<td><?php $tgl = $data['tanggal_kirim'];
									echo tgl_indo($tgl);
								 ?></td>
								...
								<td><?= $data['lokasi'] ?></td>
								<td>
									<input type="checkbox" name="checked_kirim[]" class="kirim_cek" value="<?= $data['id_kirim'] ?>">
								</td>
							</tr>
						<?php }
					}else{
...
<br>
	...
	<?php 
      ...
      ?>
        <li class="page-item"><a class="page-link" href="kirim?halaman=<?=$link_prev?>">&laquo;</a>
		<li class="page-item"><a class="page-link" href="kirim?halaman=<?=$pages?>">&lsaquo;</a>
      <?php
      }
      ?>
    	...
    	<li <?= $link_active ?>><a class="page-link" href="kirim?halaman=<?= $i; ?>"><?= $i; ?></a></li>
		<?php } ?>
      ...
      ?>
        <li class="page-item"><a class="page-link" href="kirim?halaman=<?=$link_next?>">&rsaquo;</a>
		<li class="page-item"><a class="page-link" href="kirim?halaman=<?=$pages?>">&raquo;</a>
      <?php
      }
      ?>
  </ul>
